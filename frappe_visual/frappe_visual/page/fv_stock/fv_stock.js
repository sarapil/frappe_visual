 ...
 * Inventory Dashboard
 *
 * Visual ERP dashboard for the stock module.
 * Loads the fv_erp bundle and renders the stock dashboard
 * with KPIs, charts, quick actions, and recent documents.
 * Route: /app/fv-stock
 */
frappe.pages["fv-stock"].on_page_load = function (wrapper) {
    ...
    const root = $(`<div id="fv-erp-module-root" style="padding:1rem">
        <div style="padding:3rem;text-align:center">
            <div class="fv-fx-page-enter">
                <i class="ti ti-package" style="font-size:3rem;color:var(--primary)"></i>
                <p style="margin-top:1rem;color:var(--text-muted)">${__("Loading Inventory Dashboard...")}</p>
            </div>
        </div>
    ...
    frappe.require("fv_erp.bundle.js", () => {
        ...
        try {
            if (frappe.visual.erp && frappe.visual.erp.dashboard) {
                frappe.visual.erp.dashboard(container, "stock", {});
            } else {
            ...
            console.error("FV ERP: Error loading stock", e);
            container.innerHTML = `<div style="padding:2rem;text-align:center">
