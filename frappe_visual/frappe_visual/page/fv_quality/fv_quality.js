 ...
 * Quality Management
 *
 * Visual ERP dashboard for the quality module.
 * Loads the fv_erp bundle and renders the quality dashboard
 * with KPIs, charts, quick actions, and recent documents.
 * Route: /app/fv-quality
 */
frappe.pages["fv-quality"].on_page_load = function (wrapper) {
    const page = frappe.ui.make_app_page({
        parent: wrapper,
        title: __("Quality Management"),
        single_column: true,
    });
    ...
    const root = $(`<div id="fv-erp-module-root" style="padding:1rem">
        <div style="padding:3rem;text-align:center">
            <div class="fv-fx-page-enter">
                <i class="ti ti-microscope" style="font-size:3rem;color:var(--primary)"></i>
                <p style="margin-top:1rem;color:var(--text-muted)">${__("Loading Quality Management...")}</p>
            </div>
        </div>
    ...
    frappe.require("fv_erp.bundle.js", () => {
        ...
        try {
            if (frappe.visual.erp && frappe.visual.erp.dashboard) {
                frappe.visual.erp.dashboard(container, "quality", {});
            } else {
            ...
            console.error("FV ERP: Error loading quality", e);
            container.innerHTML = `<div style="padding:2rem;text-align:center">
