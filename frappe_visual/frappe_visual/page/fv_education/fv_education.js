 ...
 * Education Dashboard
 *
 * Visual ERP dashboard for the education module.
 * Loads the fv_erp bundle and renders the education dashboard
 * with KPIs, charts, quick actions, and recent documents.
 * Route: /app/fv-education
 */
frappe.pages["fv-education"].on_page_load = function (wrapper) {
    const page = frappe.ui.make_app_page({
        parent: wrapper,
        title: __("Education Dashboard"),
        single_column: true,
    });
    ...
    const root = $(`<div id="fv-erp-module-root" style="padding:1rem">
        <div style="padding:3rem;text-align:center">
            <div class="fv-fx-page-enter">
                <i class="ti ti-school" style="font-size:3rem;color:var(--primary)"></i>
                <p style="margin-top:1rem;color:var(--text-muted)">${__("Loading Education Dashboard...")}</p>
            </div>
        </div>
    ...
    frappe.require("fv_erp.bundle.js", () => {
        ...
        try {
            if (frappe.visual.erp && frappe.visual.erp.dashboard) {
                frappe.visual.erp.dashboard(container, "education", {});
            } else {
            ...
            console.error("FV ERP: Error loading education", e);
            container.innerHTML = `<div style="padding:2rem;text-align:center">
