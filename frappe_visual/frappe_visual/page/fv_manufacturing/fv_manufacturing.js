 ...
 *
 * Visual ERP dashboard for the manufacturing module.
 * Loads the fv_erp bundle and renders the manufacturing dashboard
 * with KPIs, charts, quick actions, and recent documents.
 * Route: /app/fv-manufacturing
 */
frappe.pages["fv-manufacturing"].on_page_load = function (wrapper) {
    ...
    const root = $(`<div id="fv-erp-module-root" style="padding:1rem">
        <div style="padding:3rem;text-align:center">
            <div class="fv-fx-page-enter">
                <i class="ti ti-building-factory" style="font-size:3rem;color:var(--primary)"></i>
                <p style="margin-top:1rem;color:var(--text-muted)">${__("Loading Manufacturing Dashboard...")}</p>
            </div>
        </div>
    ...
    frappe.require("fv_erp.bundle.js", () => {
        ...
        try {
            if (frappe.visual.erp && frappe.visual.erp.dashboard) {
                frappe.visual.erp.dashboard(container, "manufacturing", {});
            } else {
            ...
            console.error("FV ERP: Error loading manufacturing", e);
            container.innerHTML = `<div style="padding:2rem;text-align:center">
