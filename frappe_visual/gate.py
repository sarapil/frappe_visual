...
"""
CAPS Permission Gate for Frappe Visual
...
def has_capability(user=None, capability=None):
    """Check if user has a specific Frappe Visual capability via CAPS."""
    if not user:
    ...
    if not capability.startswith("FV_"):
        capability = f"FV_{capability}"
